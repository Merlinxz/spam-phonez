...
def countdown_timer(seconds, label):
    ...
    for remaining in range(seconds, 0, -1):
        timer_placeholder.text(f"{label} {remaining} seconds remaining...")
        time.sleep(1)
    timer_placeholder.text(f"{label} Time's up!")
    ...
    with col2:
        if st.button("📤 Send Messages", use_container_width=True):
            if not target_numbers:
                ...
                countdown_timer(5, "Simulating message sending in")  # Example countdown before simulation
